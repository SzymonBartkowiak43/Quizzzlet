...
public interface ProgressRepository extends JpaRepository<Progress, Long> {
    ...
    @Query("SELECT p FROM Progress p WHERE p.user.id = :userId AND p.studyDate >= :startDate ORDER BY p.studyDate DESC")
    List<Progress> findRecentProgressByUserId(@Param("userId") Long userId, @Param("startDate") LocalDate startDate);
