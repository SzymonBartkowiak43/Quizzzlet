...
public class TranslationRestController {
  ...
  public ResponseEntity<ApiResponse<TranslationResponse>> translate(
      @Valid @RequestBody TranslationRequest request, Authentication authentication) {
    try {
      ...
      logger.debug(
          "Translation request from user: {} - '{}' from {} to {}",
          userEmail,
          text,
          sourceLanguage,
          targetLanguage);
      ...
      if (translation.equals(text)) {
        ...
        logger.info(
            "Translation successful for user: {} - '{}' -> '{}'", userEmail, text, translation);
        return ResponseEntity.ok(ApiResponse.success("Translation successful", response));
      }

    } catch (Exception e) {
      logger.error(
          "Error during translation for user: {} - text: '{}'",
