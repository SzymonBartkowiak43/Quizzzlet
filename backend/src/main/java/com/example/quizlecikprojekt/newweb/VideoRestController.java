...
public class VideoRestController {
    ...
    public ResponseEntity<VideosListResponse> getAllVideos(Authentication authentication) {
        ...
        VideosListResponse response = new VideosListResponse(
                allVideos.stream()
                        .map(video -> mapToVideoSummary(video, videoRatings))
                        .toList(),
                topRatedVideos.stream()
                        .map(video -> mapToVideoSummary(video, videoRatings))
                        .toList(),
                allVideos.size()
        );
    ...
    public ResponseEntity<VideosListResponse> searchVideos(
        ...
        logger.info("Video search performed by user: {} for query: '{}'",
                currentUser.getEmail(), query);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<VideoSummaryResponse> addVideo(
        ...
        VideoSummaryResponse response = mapToVideoSummary(
                createdVideo,
                Map.of(createdVideo.getId(), 0.0)
        );
    ...
    public ResponseEntity<Void> deleteComment(
        ...
        logger.info("Comment {} deleted from video {} by user: {}",
                commentId, videoId, currentUser.getEmail());
    ...
    public ResponseEntity<RatingResponse> getVideoRating(
        ...
        Optional<Integer> userRating = ratingService.getUserRatingForVideo(currentUser.getEmail(), videoId);
