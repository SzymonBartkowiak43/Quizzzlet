...
import com.example.quizlecikprojekt.newweb.dto.learn.*;
...
public class LearnRestController {
    ...
    public LearnRestController(WordSetService wordSetService, WordService wordService, UserService userService) {
    ...
    public ResponseEntity<?> startFlashcardSession(
            ...
            @Valid @RequestBody StartFlashcardRequest request) {
        ...
        if (words.isEmpty()) {
            return ResponseEntity.badRequest()
                    .body(Map.of("message", "Word set has no words to learn"));
        }
    ...
    public ResponseEntity<?> startQuizSession(
        ...
        if (allWords.isEmpty()) {
            return ResponseEntity.badRequest()
                    .body(Map.of("message", "Word set has no words for quiz"));
        }
    ...
    public ResponseEntity<?> answerQuiz(
        ...
        boolean isCorrect = currentQuestion.getCorrectAnswer().equalsIgnoreCase(request.answer().trim());

        if (isCorrect) {
            session.incrementScore();
            // Update word points
            wordService.updateWordPoints(currentQuestion.getWord().getId(),
                    currentQuestion.getWord().getPoints() + 1);
        } else {
    ...
    private FlashcardSessionResponse buildFlashcardResponse(FlashcardSession session) {
        ...
        if (!isCompleted) {
            Word currentWord = session.getWords().get(session.getCurrentIndex());
            currentCard = new FlashcardResponse(
                    currentWord.getId(),
                    currentWord.getWord(),
    ...
    private List<String> generateOptions(String correctAnswer, List<Word> allWords, boolean useWords) {
        Set<String> options = new HashSet<>();
        options.add(correctAnswer);

        List<String> candidates = allWords.stream()
    ...
    private SessionSummaryResponse buildSessionSummary(Object session, String type) {
        if (session instanceof FlashcardSession fs) {
            double accuracy = fs.getWords().size() > 0 ?
                    (double) fs.getScore() / fs.getWords().size() * 100 : 0;

            List<WordResultResponse> incorrectWords = fs.getIncorrectWords().stream()
                    .map(word -> new WordResultResponse(word.getId(), word.getWord(),
                            word.getTranslation(), false))
                    .toList();
            ...
            double accuracy = qs.getQuestions().size() > 0 ?
                    (double) qs.getScore() / qs.getQuestions().size() * 100 : 0;

            List<WordResultResponse> incorrectWords = qs.getIncorrectWords().stream()
                    .map(word -> new WordResultResponse(word.getId(), word.getWord(),
                            word.getTranslation(), false))
                    .toList();
    ...
    private static class FlashcardSession {
        ...
        public String getWordSetTitle() { return wordSetTitle; }
        public void setWordSetTitle(String wordSetTitle) { this.wordSetTitle = wordSetTitle; }
        ...
        public List<Word> getIncorrectWords() { return incorrectWords; }
        public void setIncorrectWords(List<Word> incorrectWords) { this.incorrectWords = incorrectWords; }
        public void addIncorrectWord(Word word) { this.incorrectWords.add(word); }
