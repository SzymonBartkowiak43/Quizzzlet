...
public class WordSetRestController {
    ...
    public WordSetRestController(WordSetService wordSetService, WordService wordService, UserService userService) {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<WordSetResponse>>> getWordSets(Authentication authentication) {
        try {
            ...
            List<WordSetResponse> response = wordSets.stream()
                    .map(this::mapToWordSetResponse)
                    .toList();
    ...
    public ResponseEntity<ApiResponse<WordSetResponse>> createWordSet(
            ...
            @Valid @RequestBody WordSetCreateRequest request) {
        try {
            if (authentication == null || !authentication.isAuthenticated()) {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                        .body(ApiResponse.error("User not authenticated"));
    ...
    public ResponseEntity<ApiResponse<String>> deleteWordFromWordSet(
        ...
        try {
            ...
            logger.error("Error deleting word {} from word set {} for user: {}", wordId, id, authentication.getName(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
